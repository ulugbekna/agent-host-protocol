/**
 * Canvas State Types — Local canvas identity and lifecycle exposed on
 * `ahp-canvas:` channels.
 *
 * A "canvas" is a provider-rendered surface (e.g. a local extension or
 * installed package's custom view) associated with one exact backing chat.
 * AHP models the canvas's identity, durable membership, admission/trust and
 * live availability state machine, and its declared actions — never the
 * rendering transport, page bytes, or a provider-specific SDK type. Hosts
 * remain authoritative for how a canvas is actually served; this channel
 * only carries what a client needs to discover, admit, and interact with one.
 *
